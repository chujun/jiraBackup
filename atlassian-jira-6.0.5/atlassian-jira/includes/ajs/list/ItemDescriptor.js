...
AJS.ItemDescriptor = AJS.Descriptor.extend({
    ...
    /**
     * Gets invalid, in the case of a @see AJS.QueryableDropdownSelect, a span will be added to label as a hover over the invalid
     * icon;
     *
     * @method invalid
     * @return {String}
    ...
    /**
     * AJS list looks at this to determine if it should do the highlighting/filtering or it has already beeen done
     * @return {Boolean}
     */
 ...
 * Factory method.
 *
 * This method knows how to create a AJS.ItemDescriptor and descendant classes (delegating the creation to their
 * factories)
 * @param suggestion
 * @param groupIndex
 * @return {AJS.ItemDescriptor}
 */
AJS.ItemDescriptor.create = function(suggestion, groupIndex) {
    if (suggestion.isMessage) {
        return AJS.MessageDescriptor.create(suggestion, groupIndex);
    } else {
