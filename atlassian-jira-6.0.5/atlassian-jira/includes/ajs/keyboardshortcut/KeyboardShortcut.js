...
AJS.whenIType = function(shortcut, ctx) {
    return new AJS.KeyboardShortcut(shortcut, ctx);
};
 ...
 * @param {string} shortcut
 */
AJS.KeyboardShortcut = function(shortcut, ctx) {
    this._executer = null;
    this.shortcuts = [shortcut];
    this._bindShortcut(shortcut, ctx);
};

AJS.KeyboardShortcut.prototype._bindShortcut = function(shortcut, ctx) {
 ...
 * @private
 */
AJS.KeyboardShortcut.prototype._ignoreShortcut = function (event, shortcut, ctx)  {
    var ignore = false;
    _.each(AJS.KeyboardShortcut._ignoreConditions, function (condition) {
...
AJS.KeyboardShortcut._ignoreConditions = [];
 ...
 * @param func
 */
AJS.KeyboardShortcut.addIgnoreCondition = function (func) {
    AJS.KeyboardShortcut._ignoreConditions.push(func);
};

/**
 * Ignore keyboard shortcuts when we have a dialog, dropdwon etc shown.
 */
AJS.KeyboardShortcut.addIgnoreCondition(function () {
    return AJS.popup.current || AJS.dropDown.current || JIRA.Dialog.current || AJS.keyboardShortcutsDisabled;
});

AJS.KeyboardShortcut.getKeyboardShortcutKeys = function (moduleKey) {
    for (var index in AJS.keys.shortcuts) {
        var shortcut = AJS.keys.shortcuts[index];
