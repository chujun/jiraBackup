...
AJS.InlineLayer.OptionsDescriptor = AJS.Descriptor.extend({
    ...
    init: function (properties) {
        ...
        if (!this.contentRetriever()) {
            ...
            } else if (AJS.$.isFunction(this.content())) {
        ...
        if (!this.positioningController()) {
            if (!AJS.params.ignoreFrame && this._inIFrame()) {
                ...
                if (isBodyOverflowHidden) {
                    this.positioningController(new AJS.InlineLayer.WindowPositioning());
                } else {
    ...
    content: function (content) {
        ...
        } else if (this.properties.content && (this.properties.content.length || AJS.$.isFunction(this.properties.content))) {
    ...
    offsetTarget: function (offsetTarget) {
        ...
        } else if (this.properties.offsetTarget && (this.properties.offsetTarget.length || AJS.$.isFunction(this.properties.offsetTarget))) {
     ...
     *
     * @method customEvents
     * @param {customEvents} object with custom events to add
     * @return {object}
     */
