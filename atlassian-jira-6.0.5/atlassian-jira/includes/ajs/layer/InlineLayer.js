...
AJS.InlineLayer = AJS.Control.extend({
    ...
    onerror: function (callback) {
        ...
        if (AJS.$.isFunction(callback)) {
            ...
            AJS.$.each(this.errorCallback, function (i, callback) {
                callback(instance);
            });
        }
    },
...
//                // ...but not before the current "click" live event completes, otherwise IE will not fire other click handlers.
    ...
    _assignCustomEvents: function() {
        ...
        for (var target in customEvents) {
            for (var eventType in customEvents[target]) {
                AJS.$(target).on(eventType, this._getCustomEventsDispatcher(target, eventType, customEvents));
            }
        }
    },
    ...
    _unassignCustomEvents: function () {
        ...
        for (var target in customEvents) {
            for (var eventType in customEvents[target]) {
                AJS.$(target).off(eventType, this._getCustomEventsDispatcher(target, eventType, customEvents));
            }
        }
    },
