    ...
    setup: function () {
        ...
        this.assert = {
            ...
            selected: function (group) {
                ...
                equals(JSON.stringify($select.val()), JSON.stringify(expectedVal));
            },
            ...
            suggestions: function () {
                equals(getAllSuggestions().length, arguments.length, "Expected " + arguments.length + " suggestions present");
                _.each(arguments, function (option) {
                    ok(getSuggestions(option).length === 1, "Expected suggestion \"" + option + "\" to be present");
                });
            },
            noSuggestions: function () {
                equals(getAllSuggestions().length, 0, "Expected no suggestions to be present");
                equals(getNoSuggestions().length, 1, "Expected the 'No Matches' hint to be present");
            },
            bolded: function () {
                var $bolded = getAllSuggestions().find("em");
                equals($bolded.length, arguments.length, "Expected " + arguments.length + " to be bolded");
                _.each(arguments, function (boldedText, i) {
                    equals(AJS.$.trim($bolded.eq(i).text()), boldedText, "Expected text [" + boldedText + "] to be bolded");
                });
