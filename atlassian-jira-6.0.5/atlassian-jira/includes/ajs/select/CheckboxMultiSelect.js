...
AJS.CheckboxMultiSelect = AJS.QueryableDropdownSelect.extend({
    ...
    init: function (options) {
        ...
        this.suggestionsHandler = this.options.suggestionsHandler ?
                                  ...
                                  new AJS.CheckboxMultiSelectSuggestHandler(this.options, this.model);
    ...
    _handleCharacterInput: function (force) {
        ...
        AJS.$.each(this.listController.$container.find(".invalid-item"), function() {
            AJS.$(this).tipsy("hide");
        });
     ...
     *
     * @param {AJS.ItemDescriptor} descriptor The Item Descriptor.
    ...
    _renders: {
        ...
        suggestionItem: function(descriptor, replacementText) {
            ...
            var $checkbox = AJS.$("<input type='checkbox' tabindex='-1' />").val(descriptor.value()),
                $listElem = AJS.$('<li class="check-list-item">'),
                $label = AJS.$("<label class='item-label' />"),
                $tipsyTarget,
                $img;
            ...
            if (descriptor.icon() && descriptor.icon() !== "none") {
                $img = AJS.$("<img src='" + descriptor.icon() + "' height='16' width='16' align='absmiddle' />");
