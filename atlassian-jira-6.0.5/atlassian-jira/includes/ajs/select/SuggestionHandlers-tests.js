...
test("Preserves search results items even if they already are in the suggestions", function () {
    ...
    suggestHandler.execute("query").done(
        function(descriptors) {
            equals(descriptors[0].items()[0].value(), "val1");
            equals(descriptors[0].items()[0].label(), "*Value 1*");

            equals(descriptors[0].items()[1].value(), "val2");
            equals(descriptors[0].items()[1].label(), "Value 2");

            equals(descriptors[0].items().length, 2);
        }
    )
