...
(function($) {
    ...
    module("Dialog Tests", {
        teardown: function() {
            ...
            $("."+JIRA.Dialog.ClassNames.DIALOG).remove();
        }
    });
...
(function($) {
    module("Dialog headers -- likely to change in JIRA 6.1", {
        setup: function() {
            ...
            this.dialog = new JIRA.Dialog({content: this.$content});
        }, teardown: function() {
            this.dialog.hide();
            this.$content.remove();
        }
    ...
    test("heading contents are placed inside known container", function() {
        ...
        this.dialog.addHeading(title);
        var $heading = this.dialog.$popup.find("."+JIRA.Dialog.ClassNames.HEADING_AREA);
