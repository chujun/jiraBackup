...
JIRA.Dialog = AJS.Control.extend({
    ...
    init: function (options) {
        ...
        this.classNames = JIRA.Dialog.ClassNames;
    ...
    _ellipsify: function(context) {
        if (!(context instanceof AJS.$)) {
            context = this.get$popup();
        }
        // Ellipsify dynamically loaded content
        AJS.$(".overflow-ellipsis", context).textOverflow({
    ...
    _watchTab: function(e) {
        ...
        if (AJS.$(e.target).parents(this.get$popupContent()).length > 0) {
            if (AJS.$('html').hasClass('safari')) {
                ...
                $dialog_selectable = AJS.$(':input:visible:enabled, :checkbox:visible:enabled, :radio:visible:enabled', this.OPEN_DIALOG_SELECTOR);
            } else {
                $dialog_selectable = AJS.$('a:visible, :input:visible:enabled, :checkbox:visible:enabled, :radio:visible:enabled', this.OPEN_DIALOG_SELECTOR);
            }
    ...
    hide: function (undim, options) {
        ...
        var atlToken = AJS.$("input[name=atl_token]", this.OPEN_DIALOG_SELECTOR).attr("value");
    ...
    addHeading: function(heading) {
        var $pieces = AJS.$("<div/>").html(heading).contents();
        var $title = AJS.$("<h2/>");
        var contents = [];
        ...
        $title.attr('title', AJS.$.trim($title.text()));
    },
