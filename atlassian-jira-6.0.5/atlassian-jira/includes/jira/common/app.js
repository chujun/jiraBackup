    ...
    JIRA.bind(JIRA.Events.NEW_CONTENT_ADDED, function(event, $context) {
        ...
        $context.find("#filter-edit").on("submit", function(event) {
            if (!event.isDefaultPrevented()) {
                ...
                JIRA.Messages.showMsgOnReload(AJS.I18n.getText("editfilter.save.success", filterName), {
                    type: "SUCCESS",
                    closeable: true,
        ...
        $context.find('#filter-subscription').on("submit", function(event) {
            if (!event.isDefaultPrevented()) {
                ...
                JIRA.Messages.showMsgOnReload(AJS.I18n.getText("subscriptions.add.success", AJS.escapeHtml(recipient)), {
                    type: "SUCCESS",
                    closeable: true,
