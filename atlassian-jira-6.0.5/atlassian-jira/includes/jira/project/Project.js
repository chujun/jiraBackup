...
JIRA.Project = (function() {
    ...
    return {
        ...
        navigationTabs: function () {
            ...
            unloadEvents = {},// {Object} - contains a hash map of functions to be executed after specified tabs are unloaded
            ...
            /**
             * @function {Private} runTabUnloadEvent
             * @param {String} hashMapID
             */
            runTabUnloadEvent = function (hashMapID) {
                if (unloadEvents[hashMapID] && unloadEvents[hashMapID] instanceof Array) {
                    jQuery(unloadEvents[hashMapID]).each(function () {
                        this();
                    });
                }
            },
            ...
            navigateToTab = function (tab, historyEvent, url) {
                ...
                populateTab = function (contentObj, textStatus, xhr) {
                    ...
                    if (permissionsCheck(xhr, contentObj, tabAnchor.attr("href"))) {
                        ...
                        runTabUnloadEvent(previousTab.find("a").attr("id"));
                        projectTab.empty().append(contentObj.content);
                        if (!historyEvent) {
                        ...
                        JIRA.trigger(JIRA.Events.NEW_CONTENT_ADDED, [projectTab, JIRA.CONTENT_ADDED_REASON.tabUpdated]);
                    }
                };
            ...
            return {
                ...
                /**
                 * Events to call after a tab is unloaded.
                 * @function addUnloadEvent
                 * @return {Object}
                 */
                addUnloadEvent: function (tabName, handler) {
                    unloadEvents[tabName] = unloadEvents[tabName] || [];
                    if (jQuery.isFunction(handler)) {
                        unloadEvents[tabName].push(handler);
                    }
                },
...
jQuery(document).ready(JIRA.VersionBlocks.init);

// Initialise the EditProject dialog. Uses delegation on document so no need to reinitialise.
JIRA.createEditProjectDialog("#edit_project");

// Refresh the panel on creation of a new issue.
JIRA.bind("QuickCreateIssue.sessionComplete", function () {
    JIRA.Project.navigationTabs.getActiveTab().find("a").click();
});
