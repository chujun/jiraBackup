...
JIRA.ViewIssueTabs = (function() {
            ...
            var xhr = JIRA.SmartAjax.makeRequest({
                ...
                complete: function (xhr, status, smartAjaxResult) {
                    if (status != 'abort') {
                        ...
                        dispatchIssueTabLoadedEvent(container);
    ...
    function smartUpdate(container, newElements) {
        ...
        var newContents = newElements.find('.issuePanelContainer');
        ...
        if (newTabs.length && oldTabs.length && newContents.length && oldContents.length) {
            ...
            var newContentHeight = newContents.height();
            ...
            if (visibleHeightDifference > 0) {
                ...
                // Delay the animation so the user has time to register that it is shrinking due to
                // the extra whitespace.
                setTimeout(function() {
                    var animationSpeed = 500; // pixels/second
                    $tabContents.animate({
                        height: newContentHeight
                    }, visibleHeightDifference / animationSpeed * 1000, 'easeOutQuart', function() {
